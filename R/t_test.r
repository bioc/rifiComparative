#loop into the fragments and extract the values for t-test. The values are either
#distance of half-life or intensity.
t_test_function <- function(data, par, par1, cdt1, cdt2, frag){
    ...
    for (i in seq_along(frag)) {
        ...
        par.cd2 <- data[which(data[,paste0(par,"_comb_fragment")] == frag[i]),
                        paste0(par1, ".",cdt2)]
        if (length(par.cd1) < 3 | length(par.cd2) < 3 | par.cd1 == par.cd2) {
            next ()
        } 
        t_h <-
            t.test(par.cd1,
